	...
	methods: {
		...
		getContent() {
			this.$prismic.client
				...
				.then((document) => {
					let array = [];
					var size = Object.keys(document.results[15].data).length;
					for (let i = 0; i < size; i++) {
						array.push(Object.values(document.results[15].data)[i]);
					}
					...
					this.fields.name = document.results[15].data.title[0].text;
				});
		},
	},
	created() {
